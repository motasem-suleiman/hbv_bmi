...
from RoutingRoutine import HBV_ROUTING


class HBV_ROUTINGBmi(Bmi):

    """Temperature index snow model"""

    _name = "Hydrologiska Byråns Vattenbalansavdelning Model with BMI"
    _input_var_names = ("land_surface_water__baseflow_volume_flux",
                       )
    ...
    def initialize(self, filename=None):
        ...
        """
        if filename is None:
            self._model = HBV_ROUTING()
        elif isinstance(filename, str):
            with open(filename, "r") as file_obj:
                self._model = HBV_ROUTING.from_file_like(file_obj.read())
        else:
            self._model = HBV_ROUTING.from_file_like(filename)

        self._values = {"land_surface_water__baseflow_volume_flux": self._model.Qgen_mm,
                        "land_surface_water__runoff_volume_flux": self._model.Qgen_routed_mm,}
        
        self._var_units = {"land_surface_water__baseflow_volume_flux": "mm",
